...
class RollingWeaveNode:
	...
	def laser_callback(self, msg):
		if str(self.state) == "UPS": #change back to rolling weave node
			self.started = True
			...
			speed_potential = 1/(2*abs(raw_turn))+.3
			if(speed_potential > 3 ): # max 5
				speed_potential = 3
			...
			self.last_msg = drive_msg

		elif self.started and self.extend_time:
			return
			for x in range (0, 30):
				rospy.logwarn("ghetto part now take caution")
				first_ind, last_ind = self.get_index(140, msg)
				frontf_ind, frontl_ind = self.get_index(100, msg)
				front_potential = msg.ranges[frontf_ind:frontl_ind] 
				left_potential = msg.ranges[first_ind:int(first_ind + (last_ind - first_ind)/2)]
...
if __name__ == "__main__":
	rospy.init_node("Rolling_Weave")
	node = RollingWeaveNode()
	rospy.spin()
