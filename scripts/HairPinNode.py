...
class HairPinNode.py
	self.state = None

	def __init__(self):
		ropsy.Subscriber("/state", String, self.state_callback)
	...
	def laser_callback(self, msg):
		if self.state = "HPS":
			...
			speed_potential = 1/(3*abs(raw_turn))+.3
			if(speed_potential > 3.5 ):
				speed_potential = 3.5
